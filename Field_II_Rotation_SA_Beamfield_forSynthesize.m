...
aPosRng = linspace(-0.5*(stTxInfo.nNoTx-1), 0.5*(stTxInfo.nNoTx-1), stTxInfo.nNoTx) * nFOV_Theta / stTxInfo.nNoTx;

%% start index
disp(['Total Tx is ' num2str(numel(aPosRng))]);
start_idx = input('Enter the start index: ');
%% Save Directory Setting
% sSaveDir = [sCurrentPath '/beamfield/FOV' num2str(nFOV_Theta) '_Radius' num2str(nRadius*1e3) 'mm_Tx' num2str(nNoTx) '_Foc' num2str(stTRInfo.nEleFocus*1e3) 'mm'];
sSaveDir = uigetdir('','');
% mkdir(sSaveDir);
...
for pIdx = start_idx:nNoTx
    ...
    save([sSaveDir '/[Beamfield]Tx_Index_' num2str(pIdx,'%.4d')],'stPressure','-v7.3');
