...
stTxInfo.cycle = 1;
stTxInfo.num_tx_vol = 2047; % volume scan
...
if(~load_scatter)
    ...
    for d_idx = 1:numel(depth_pos)
        ...
        radial_pos_tmp = radial_pos(d_idx,:);
        
        deg_tmp = asind(radial_pos_tmp/(depth_tmp + radius));
        
        lat_tmp = (radius + depth_tmp) * sind(deg_tmp);
        ver_tmp = (radius + depth_tmp) * cosd(deg_tmp) - radius;
        
        for p_idx = 1:size(radial_pos,2)
            m_point_pos = cat(1,m_point_pos, [0 lat_tmp(p_idx) ver_tmp(p_idx)]);
        end
    end
