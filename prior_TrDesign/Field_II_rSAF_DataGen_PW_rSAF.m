...
% Data gene  ration for 3D volumetric image
...
load_scatter = 1; scatter_name_l = 'prostate_mimic';
save_scatter = 0; scatter_name_s = 'sphere';

field_init(0);

% processing_scanlines = 784:1:860;
processing_scanlines = 321;
...
if(~load_scatter)
    ...
    dir_scat = [sCurrentPath '/scatterer'];
    load([dir_scat '/' scatter_name_l]);
...
if(numel(raw_list)>=3) % if already processed planes
    ...
    last_done_scanline = split(raw_list(end).name,'_');
    last_done_scanline = str2double(last_done_scanline{end});
    start_p_idx = find(processing_scanlines == last_done_scanline);
else % if it is first
    start_p_idx = 1;
end
...
if(start_p_idx == 1)
    ...
    last_done_plane = ['RAW_' num2str(last_done_scanline,'%.4d')];
...
for p_idx = start_p_idx:numel(processing_scanlines)
    disp(['>>> Volume scan [' num2str(processing_scanlines(p_idx)) '/' num2str(stTxInfo.num_tx_vol) ']...']);
    % Field rotation
    nTheta = aPosRng(processing_scanlines(p_idx));
    ...
    dir_tmp = [save_dir_data '/RAW_' num2str(processing_scanlines(p_idx), '%.4d')];
    mkdir(dir_tmp);
