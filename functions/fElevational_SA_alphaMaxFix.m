function [mBFedData, vBFedData, vSynReg] = fElevational_SA_alphaMaxFix(vRcvData, stRFInfo, stBFInfo, stTRInfo, stSAInfo, aSclineTheta, mPosY, mPosZ, nDelayOff, idx, syn_idx)
...
%% get delta_theta, interval
num_syn = stSAInfo.nNoSyn;
interval_ = stSAInfo.interval;
%%
...
%     disp(['scanline: ' num2str(s_idx)]);
    ...
    % get beam index
%     beam_idx = max(s_idx - 0.5*(num_syn-1)*interval_,1):interval_:min(s_idx + 0.5*(num_syn-1)*interval_,nNoTx);
    beam_idx = (s_idx-0.5*(num_syn-1)*interval_):interval_:(s_idx+0.5*(num_syn-1)*interval_);
    beam_idx = beam_idx(beam_idx>0 & beam_idx<=stBFInfo.nScline);
    for b_idx = beam_idx % include center beam
        % Transmit delay
