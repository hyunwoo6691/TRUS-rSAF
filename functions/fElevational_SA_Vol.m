function [mBFedData, vBFedData, vSynReg, vApod] = fElevational_SA_Vol(mBFedDataLat, stRFInfo, stBFInfo, stTRInfo, stSAInfo, aSclineTheta, mPosY, mPosZ, nDelayOff)
[nNoSpl, nNoTx] = size(mBFedDataLat);
%% Element position
...
%% Beamforming
vBFedData = zeros(stBFInfo.nDthSpl, nNoTx, stBFInfo.nRadialScline);
mBFedData = zeros(stBFInfo.nDthSpl, stBFInfo.nRadialScline);
vSynReg = zeros(stBFInfo.nDthSpl, stBFInfo.nRadialScline, nNoTx);
vApod = zeros(stBFInfo.nDthSpl, nNoTx, stBFInfo.nRadialScline);

for s_idx = 1:stBFInfo.nRadialScline
...
%                 aApod = (dist_to_scline <= 0.5 * aAptSize) .* (0.5*cos(2*pi*dist_to_scline ./ aAptSize));
        ...
        aAddr = max(min(aAddr,size(mBFedDataLat,1)-1),1);
        aLogic = (aAddr>0).*(aAddr<size(mBFedDataLat,1)-1);
        
        aInterpData = interpn(0:1:size(mBFedDataLat,1)-1,mBFedDataLat(:,b_idx),aAddr,'spline');
