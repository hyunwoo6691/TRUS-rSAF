...
dir_tmp = [dir_ '/errors_bf/error_0/Sample001/Element_064'];
...
%% plot FWHM
% plot_syn = [3 5 19 65];
% legend_ = cell(numel(plot_syn)+1, 1);
% axis_ = (1:nNoPT)*10;
% figure(1010);
% % for CON
% plot(axis_, FWHM_CON, 'LineWidth', 2); hold on;
% legend_{1} = 'CON';
% % for rSAF
% for s_idx = 1:numel(plot_syn)
%     idx_tmp = plot_syn(s_idx);
%     idx_tmpp = find(idx_tmp == num_syn);
%     plot(axis_, FWHM_rSAF(idx_tmpp, :), 'LineWidth', 2);
%     legend_{s_idx+1} = strcat('rSAF-', num2str(plot_syn(s_idx)));
% end
% hold off;
% xlabel('axial [mm]'); ylabel('FWHM [mm]');
% legend(legend_, 'Location', 'northwest');
% set(gcf, 'Position', [1109 614 641 314]);

%% plot spatial resolution (num_syn vs point target)
figure(1011);
imagesc(1:nNoPT, num_syn, FWHM_rSAF);
