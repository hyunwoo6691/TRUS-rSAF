...
% num_syn = 131;
% num_syn = [3 5 7 9 11 13 15 17 19];
num_syn = 7;
...
if(bAWGN)
%     no_samples = 100;
    no_samples = 1; % error = 0 (ground truth)
else
    no_samples = 1;
end
...
stBFInfo.nDTheta = abs(aPosRng_fieldII(1) - aPosRng_fieldII(2));
...
for s_idx = 1:no_samples
    ...
    for e_idx = 1:numel(error_case) % ignore this for-loop since we have no error in scanning
        ...
        for t = 1:scline
            ...
            % load raw data
            load([dir_data '/' data_list(idx).name '/raw_data.mat']); % get planewave transmitted at 0 deg
            %                 load([dir_data '/' data_list(idx).name '/Scanline_006.mat']); % get planewave transmitted at 0 deg
            vRcvData = cat(3, vRcvData, mRcvDataTmp);
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                         nNaturalFocalDth = stTRInfo.nHeight^2/(4*stRFInfo.nLambda);
                        nNaturalFocalDth = stTRInfo.nRadius^2/(stRFInfo.nLambda);
