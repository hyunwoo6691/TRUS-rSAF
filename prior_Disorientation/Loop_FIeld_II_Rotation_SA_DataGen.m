...
sSaveDir = '/Users/songhyunwoo/Documents/JohnsHopkins/Musiic/Research/Elevational SA/01.Data/';

aEleFocus = [0, 3e-3, 6e-3];
aNoTx = [128];
...
for a = 1:numel(aEleFocus)
    for b = 1:numel(aNoTx)
        ...
        if(strcmp(sDirection,'elevational'))
            ...
            nRadius = 5e-3;
            nFOV_Theta = 60;
            aPosRng = linspace(0.5* nFOV_Theta, -0.5*nFOV_Theta, stTxInfo.nNoTx);
            %%%%%%%%%%%%
        end
...
%         axis equal; axis tight; 
        ...
        if(strcmp(sDirection, 'elevational'))
            sFolderName = ['[' sDirection ']Focus' num2str(stTRInfo.nEleFocus*1e3) 'mm_FOV' num2str(nFOV_Theta) '_R' num2str(nRadius*1e3) 'mm_Tx' num2str(stTxInfo.nNoTx)];
        else % lateral
