...
%         mkdir(dir_savee);
        ...
        for s_idx = last_done:numel(sample_list)
            ...
            element_list = dir(dir_tmpp);
            flag = 0;
            if(strcmp(element_list(3).name, '.DS_Store')), flag = 1; end
            element_list = element_list(3+flag:end);
            
            dir_saveee = [dir_savee '/' sample_name];
            mkdir(dir_saveee);
            for ele_idx = 1:numel(element_list)
                element_tmp = element_list(ele_idx).name;
                dir_tmppp = [dir_tmpp '/' element_tmp];
                
                disp(['        Element # : ' num2str(ele_idx)]);
                file_list = dir(dir_tmppp);
                flag = 0;
                ...
                % save
                if(bSave)
                    save([dir_saveee '/' element_tmp],'mLateralResol');
                end
                elapsed_time = toc;
