...
sSaveDir_master = [sCurrentPath '/01.Data/'];
...
%     stTRInfo.nEleFocus              = stTRInfo.nHeight^2/(4*stRFInfo.nLambda);
...
%     nNaturalFocalDth                = stTRInfo.nHeight^2/(4*stRFInfo.nLambda);
    ...
    if(strcmp(sDirection,'lateral'))
        ...
        stTRInfo.nNoEle                 = 1;
        stTxInfo.nTxFocus               = stTRInfo.nEleFocus;
    end
    ...
    if(strcmp(sDirection,'elevational'))
        ...
        mean_ = 0;
        sigma_ = 0.01;
        aError =  nError_range * ones(1, stTxInfo.nNoTx); % fixed error
%         aError =  2*nError_range* rand(1,stTxInfo.nNoTx) - nError_range; % normal error
%         aError = nError_range + normrnd(mean_, sigma_, [1,stTxInfo.nNoTx]); % gaussian error
        aPosRng_ground_truth = linspace(0.5* nFOV_Theta, -0.5*nFOV_Theta, stTxInfo.nNoTx);
        
        if(nError_range == 0)
            aPosRng = aPosRng_ground_truth;
        else
...
%     figure(3);scatter(mScatXYZPos_Rot(:,2), mScatXYZPos_Rot(:,3),mScatMag,'filled');
%     % xlabel('Lateral');
%     xlabel('Elevational');
%     ylabel('Axial');
%     axis equal; axis tight; grid on;
    ...
    if(strcmp(sDirection,'elevational'))
        ...
        stSaveInfo.aError = fliplr(aError);
    end
    ...
    if(strcmp(sDirection, 'elevational'))
        ...
        sFolderName = ['[' sDirection ',' stTRInfo.sType ']Focus' num2str(stTxInfo.nTxFocus*1e3) 'mm_Fnum' num2str(stTxInfo.nTxFnum) '_Tx' num2str(stTxInfo.nNoTx) '_ErrorRange_' num2str(nError_range)];
    end
    
	sSaveDir = [sSaveDir_master sFolderName '/Sample_' num2str(nIdx, '%.3d')];
	mkdir(sSaveDir)
    
    save([sSaveDir '/vRcvData'],'vRcvData');
    save([sSaveDir '/stSaveInfo'], 'stSaveInfo');
